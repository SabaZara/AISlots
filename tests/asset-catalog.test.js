...
test("World Forge publishes all 1,344 player-selectable combinations", () => {
  ...
  assert.equal(VISUAL_COMBINATION_COUNT, 7 * 8 * 4 * 6);
  assert.equal(VISUAL_COMBINATION_COUNT, 1344);
  assert.deepEqual(THEMES.map(({ id }) => id), ["fire", "ice", "nature", "void", "reef", "temple", "eclipse"]);
...
test("each generated raster asset is local and production-sized", async () => {
  ...
  assert.equal(new Set(items.map((item) => item.asset)).size, 25);
  for (const item of items) {
    ...
    if (item.asset.endsWith(".png")) {
      ...
      if (COMPANIONS.includes(item) || SYMBOL_SETS.includes(item)) {
...
test("each symbol family uses a transparent seven-symbol atlas", async () => {
  ...
  for (const set of SYMBOL_SETS) {
    assert.match(set.asset, /^\.\/assets\/factory\/symbols-.+-transparent-v3\.png$/);
...
test("each world has dedicated bonus loading art and a compatible transparent plane livery", async () => {
  assert.equal(new Set(THEMES.map((theme) => theme.bonusLoading)).size, 7);
  for (const theme of THEMES) {
    assert.match(theme.bonusLoading, /^\.\/assets\/factory\/bonus-loading-.+-v1\.(?:jpg|png)$/);
    ...
    if (theme.bonusLoading.endsWith(".png")) {
      assert.deepEqual(Array.from(loading.subarray(0, 8)), [137, 80, 78, 71, 13, 10, 26, 10]);
    } else {
