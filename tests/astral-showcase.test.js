...
test("phone spin, bet sizing, and feature controls stay visible without RTP or MAX clutter", async () => {
  ...
  assert.match(css, /4\.8\.10 creator selection fix[\s\S]*?factory-builder \.factory-options button\.is-selected::after[\s\S]*?content: none !important/);
  assert.match(css, /4\.8\.10 creator selection fix[\s\S]*?brand > \.brand-mark[\s\S]*?display: grid !important/);
  assert.match(css, /4\.8\.10 creator selection fix[\s\S]*?#maxBetButton[\s\S]*?display: none !important/);
  assert.match(css, /4\.8\.10 creator selection fix[\s\S]*?#autoButton[\s\S]*?pointer-events: auto !important/);
  assert.match(css, /4\.8\.10 creator selection fix[\s\S]*?#betDown:hover:not\(:disabled\)[\s\S]*?transform: none !important/);
  assert.match(css, /4\.8\.10 creator selection fix[\s\S]*?phoneReelStartLeftToRight[\s\S]*?animation-delay: var\(--reel-start-delay/);
  assert.match(css, /4\.8\.10 creator selection fix[\s\S]*?grid-template-columns: minmax\(0, 1fr\)[\s\S]*?left: -112px !important/);
  assert.match(css, /4\.8\.10 creator selection fix[\s\S]*?#autoButton[\s\S]*?left: calc\(50% \+ 32px\) !important/);
  assert.match(css, /4\.8\.10 creator selection fix[\s\S]*?data-factory-step="theme"[\s\S]*?brightness\(\.84\) saturate\(\.88\)[\s\S]*?button\.is-selected:active[\s\S]*?transform: none !important/);
