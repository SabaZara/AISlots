...
test("separate autoplay and speed buttons plus every positive payout remain visible", async () => {
  ...
  assert.match(css, /machine > \.companion-stage[\s\S]*?right: clamp\(-160px, -11vw, -90px\)[\s\S]*?width: clamp\(220px, 21vw, 300px\)/);
  ...
  assert.match(css, /grid-template-columns: 82px minmax\(190px, 1fr\) 220px minmax\(200px, 1fr\)/);
  assert.match(css, /padding-left: clamp\(250px, 21vw, 330px\)[\s\S]*?padding-right: clamp\(142px, 12vw, 180px\)/);
